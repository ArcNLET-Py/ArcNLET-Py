...
arcpy.env.overwriteOutput = True # Allows data to be overwriten
...
class DarcyFlow:
    ...
    def smoothDEM(self, raster, factor, flag_fsink=False):
        ...
        arcpy.env.workspace = "in_memory"  # Set the workspace to in-memory to speed up the process
        smoothed_dem = raster
        for i in range(factor):
            smoothed_dem_name = "in_memory/smoothed_dem_{}".format(i)  # Give a unique name for each iteration
        ...
        if flag_fsink:
            # Fill sinks
            smoothed_filled_dem_name = "in_memory/smoothed_filled_dem"
